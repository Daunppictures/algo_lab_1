...
def merge(left, right):
    ...
    while left_index<len(left) and right_index<len(right):
        if left[left_index].max_number_of_concurrent_visitors > right[right_index].max_number_of_concurrent_visitors:
...
def merge_sort_by_visitors(pools):
    ...
    left, right = merge_sort_by_visitors(pools[:len(pools)//2]), merge_sort_by_visitors(pools[len(pools)//2:])
